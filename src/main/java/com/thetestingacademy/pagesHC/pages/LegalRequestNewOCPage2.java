    ...
    public void fillNewOCRequest() {
        ...
        Allure.step("Entering Request Matter", () -> {
            ...
            WebElement requestMatterNewTxtBox = wait.until(
                    ExpectedConditions.visibilityOfElementLocated(requestMatter)
            );

            ((JavascriptExecutor) driver).executeScript(
                    "arguments[0].scrollIntoView({block: 'center'});",
                    requestMatterNewTxtBox
            );

            wait.until(
                    ExpectedConditions.elementToBeClickable(requestMatterNewTxtBox)
            );

            requestMatterNewTxtBox.click();

            requestMatterNew = ConfigReader.getData("newOC.requestmatter");
        ...
        Allure.step("Enter OC Firm Name", () -> {
            ...
            scrollToElement(ocFirmLocator);

            click(field);

            type(ocFirmLocator, ocFirmName);
        ...
        Allure.step("Entering Justification", () -> {
            ...
            scrollToElement(ocJusLocator);

            click(field);

            type(ocJusLocator, ocJustification);
        ...
        Allure.step("Capture NEWOC Request Number from confirmation screen", () -> {
            ...
            System.out.println("EX OC Request Number : " + newOCRequestNumber);

            Allure.addAttachment("EX OC Request Number", newOCRequestNumber);
        });
